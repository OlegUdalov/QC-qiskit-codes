...
def exc_yordanov_single_only_no_ladder_red(n_qubits, theta, qc, n_l):
    ...
    the ansatz allows to reduce number of transitions depending on the transition distance. First goes transitions between neighbouring qubits. 
    Next the next neighbours and etc. n_l is the farest transition we do.
    ...
    n = 0
    for i in range(n_l):
        for j in range(n_qubits - n_l):
            qc = se_yordanov_no_ladder(theta[n], j, j + i, qc)
            n = n + 1
